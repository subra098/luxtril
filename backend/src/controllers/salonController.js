...
exports.getMySalon = async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT s.*, json_agg(json_build_object('day_of_week', wh.day_of_week, 'is_open', wh.is_open, 'open_time', wh.open_time, 'close_time', wh.close_time, 'manual_slots', wh.manual_slots) ORDER BY wh.day_of_week) as working_hours
  ...
  try {
    ...
    try {
      ...
      for (const h of hours) {
        await client.query(
          `INSERT INTO working_hours (salon_id, day_of_week, is_open, open_time, close_time, manual_slots) 
           VALUES ($1, $2, $3, $4, $5, $6) 
           ON CONFLICT (salon_id, day_of_week) 
           DO UPDATE SET is_open = EXCLUDED.is_open, open_time = EXCLUDED.open_time, close_time = EXCLUDED.close_time, manual_slots = EXCLUDED.manual_slots`,
          [salonId, h.day_of_week, h.is_open, h.open_time || '09:00', h.close_time || '21:00', h.manual_slots || []]
        );
      }
...
exports.getAvailableSlots = async (req, res) => {
  try {
    ...
    const slots = manualSlots.map(time => {
      const timeStr = time.substring(0, 5); // Ensure HH:MM
