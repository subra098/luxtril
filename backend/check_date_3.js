...
pool.query("SELECT COALESCE(SUM(total_amount) FILTER (WHERE booking_date = $1 AND status IN ('completed', 'confirmed', 'accepted')), 0) as today_revenue FROM bookings", [todayUTC])
